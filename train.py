...
from model2 import PatchDiscriminator, ResUnet
...
def train() :
    ...
    #     writer.add_scalar("train_D(val)", D_avg_loss, epoch)
        ...
        epoch_h = epoch + 1
        if epoch_h > 0:
            torch.save(G.state_dict(), '/root/autodl-tmp/GAN/nores_Model/netG_epoch_%d.pth' % epoch_h)
            torch.save(D.state_dict(), '/root/autodl-tmp/GAN/nores_Model/netD_epoch_%d.pth' % epoch_h)
    ...
    try:
        # 保存优化器
        torch.save(optG.state_dict(), '/root/autodl-tmp/GAN/nores_Model/optG_epoch_%d.pth' % epoch_h)
        torch.save(optD.state_dict(), '/root/autodl-tmp/GAN/nores_Model/optD_epoch_%d.pth' % epoch_h)
        time.sleep(4)
    except:
